...
describe 'Brackets/event', ->
	...
	it 'parse should set the correct teams',->
		matchupTeams = @model.get('matchup').get 'teams'
		expect(matchupTeams[0].get 'name').to.eql "Acer"
		expect(matchupTeams[1].get 'name').to.eql "NSHoSeo"
